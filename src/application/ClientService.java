...
import domain.model.CallAcceptRequest;
import domain.model.CallAcceptResponse;
import domain.model.CallIdRequest;
import domain.model.CallMessageResponse;
import domain.model.CallRejectRequest;
import domain.model.CallRejectResponse;
import domain.model.DrivingInfoResponse;
import domain.model.DrivingStatus;
import domain.model.DrivingSummaryResponse;
...
public class ClientService {
	...
	// 하차 응답
	private void subscribeDrivingResult() {
		System.out.println("하차 응답 구독");

		stompSession.subscribe("/user/queue/driving-result", new StompFrameHandler() {
			@Override
			public Type getPayloadType(StompHeaders headers) {
			...
			@Override
			public void handleFrame(StompHeaders headers, Object payload) {
				System.out.println("하차 응답 호출");

				if (payload instanceof DrivingSummaryResponse summary) {
	...
	public void sendRideFinish(Long callId) {
		...
		System.out.println("하차 요청 callId : " + request.getCallId());
		...
		scheduler.scheduleAtFixedRate(() -> {
			try {
				...
				if (stompSession != null && stompSession.isConnected()) {
					...
					System.out.println(
						"location push : (" + location.getLatitude() + ", " + location.getLongitude() + ") "
							+ currentCustomerLoginId);
				}
			} catch (Exception e) {
